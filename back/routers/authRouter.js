...
// /api/auth/login 處理買家登入的網址
router.post('/login', async (req, res, next) => {
  // 確認 買家account 是否存在
  let [members] = await pool.execute('SELECT * FROM users WHERE users_valid_role=0 && users_account = ?', [req.body.account]);
  if (members.length === 0) {
...
router.post('/Artistlogin', async (req, res, next) => {
  // 確認 買家account 是否存在
  let [members] = await pool.execute('SELECT * FROM users WHERE users_valid_role=1 && users_account = ?', [req.body.account]);
  if (members.length === 0) {
    // 表示這個 account 不存在資料庫中 -> 沒註冊過
    // 不存在，就回覆 401
    return res.status(401).json({
